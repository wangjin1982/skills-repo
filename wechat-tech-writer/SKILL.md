...
1. **📏 标题长度限制（重要！）**
   - **微信公众号限制：标题最长32字节**（不是32个字符！）
   - **中文字符 = 3字节**，所以标题最多约10个汉字
   - ❌ 错误：`"如何使用Claude Skills搭建你的数字员工：从零到一的完整指南"`（43字节）
   - ✅ 正确：`"Claude Skills实战指南"`（25字节）
   - 💡 建议：控制在8-10个汉字，保留核心关键词
...
根据内容类型选择对应的结构模板（位于 `templates/` 目录）：

**AI工具实战类**：
- `tool_deep_dive.md` - 单工具深度解析（1550字）
- `tool_workflow.md` - 工具组合工作流（1500字）

**创业方法论类**：
- `startup_knowledge.md` - 创业必知必会（1700字）
- `startup_pitfalls.md` - 创业避坑指南（1600字）

**效率提升类**：
- `workflow_optimization.md` - 工作流优化案例（1600字）
- `time_management.md` - 时间管理方法（1400字）

**个人成长类**：
- `skill_development.md` - 能力提升路径（1500字）
- `mindset_adjustment.md` - 心态调整指南（1500字）

**案例分析类**：
- `success_case.md` - 成功案例拆解（1400字）
- `failure_analysis.md` - 失败案例分析（1400字）

**资源整合类**：
- `tool_collection.md` - 工具/资源推荐合集（1600字）
- `template_sharing.md` - 模板/清单分享（1500字）

**使用方法**：
1. 根据话题确定内容类型
2. 阅读对应模板文件，了解结构要求
3. 严格按照模板的章节结构和字数要求创作
4. 参考模板中的"写作要点"确保质量
...
技术类文章应该包含实际可用的代码示例，避免空洞承诺"附上代码"但不实际提供。以下场景必须使用代码块：

1. **安装/卸载指令**：工具的安装、卸载命令
   - 示例：`pip install tool-name`、`npm uninstall package`

2. **基本使用命令**：工具的核心命令和参数
   - 示例：`tool-name --help`、`git clone repo-url`

3. **配置文件示例**：配置文件的结构和关键配置项
   - 示例：JSON配置、YAML配置、环境变量设置

4. **API调用示例**：API的请求和响应示例
   - 示例：HTTP请求、SDK调用代码
...
1. **使用Markdown代码块语法**：\`\`\`语言名称
   - 必须指定语言（python、javascript、bash、json、yaml等）
   - 这样可以启用语法高亮，提升可读性
...
- ✅ **必须提供实际代码**：避免说"附上代码"但不实际提供
- ✅ **代码要实用**：读者可以直接复制使用
- ✅ **格式要规范**：使用正确的Markdown语法和语言标识
- ❌ **避免过长代码**：超过20行的代码应该精简或分段展示
...
font_paths = [
    ...
    "/usr/share/fonts/opentype/noto/NotoSansCJK-Regular.ttc",  # Noto黑体
...
# 创建图片并绘制中文
img = Image.new('RGB', (900, 500), '#1a1f5c')
draw = ImageDraw.Draw(img)
draw.text((450, 50), "中文标题", fill='white', font=title_font, anchor='mm')
img.save('cover.png')
```
